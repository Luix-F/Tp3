...
public class tp1 {
        ...
        do {
            ...
            switch (key) {
                ...
                case 23: // RSA Cript
                    sra.PainelSraCripto();
                    break;
                case 24: // RSA DesCript
                    sra.PainelSraDescripto();
                    break;
                case 86: // Desativado
                    //case86();
                    break;
    ...
    public static void garavTemp(long tmp, int p) {
        ...
        if (p <= 3) {
            ...
            System.out.println("ms\n");
            System.out.print("Tempo de execução de lzw Compactando = ");
            System.out.print(lzw);
            
            System.out.println("ms");
            System.out.print("Tempo de execução de lzw Descompactando = ");
    ...
    public static void menu(int page) {
        if (page == 1) {
            ...
            System.out.println(" -Protocolo de Criptorafia-------------|--------------------------------------");
            System.out.println(md1 + "Menu:       " + info);
            System.out.println("|                                      |                                      |");
            System.out.println("| 23: Criptografia em RSA              |                                      |");
            System.out.println("| 24: Descriptografia em RSA           |                                      |");
            System.out.println("|                                      |                                      |");
            System.out.println("|                                      |                                      |");
            System.out.println("| 7: Pagina anterior                   |                                    6 |");
            System.out.println(" -----------------------------------------------------------------------------");
            System.out.print(": ");
        }
    }
...
class sra {
    ...
    public void PainelSraCripto() {
        //String arquivo = "NATAL";
        String arquivo = CarregarArq(tp1.md2);
        String encriptado = "";

        int p = 97;  // Banco de chaves para o RSA2
        int q = 107;
        int n = p*q;
        int z = (p-1)*(q-1);
        int d = primoEmRelacao(z); // pega o maior numero primo em relacao  Z
        int e = functionE(d,z);

        int c;
        for (int i = 0; i < arquivo.length(); i++) {
            c = arquivo.charAt(i);
            encriptado += A_elev_B_mod_M(c,e,n); // (A^B)mod M
            encriptado += " ";                  // Separador
        }

        GravaCript(encriptado, "./cript/RSA.txt");
        //System.out.println("Fim: " + encriptado);
    }

    public void PainelSraDescripto() {
        //String encriptado = "5 1 14 1 12 ";
        String encriptado = CarregarArq("./cript/RSA.txt");
        String descritp = "";

        int p = 97; // Banco de chaves para o RSA
        int q = 107;
        int n = p*q;
        int z = (p-1)*(q-1);
        int d = primoEmRelacao(z); // pega o maior numero primo em relacao  Z
        ...
        for (int j = 0; j < encriptado.length(); j++) {
            if (encriptado.charAt(j) != ' ') {
                montador += encriptado.charAt(j);
            }else{
                utilizavel = Integer.parseInt(montador);
                montador = "";
                c = A_elev_B_mod_M(utilizavel, d, n);
                descritp += (char) c;
            }
        }
        //System.out.println("Fim: " + descritp);
        GravaCript(descritp, "./cript/RSA-descript.txt");
    }



    public String CarregarArq(String loc){// Carrega arquivo de texto
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("loadArq <- RSA");
        }
        return line;
    }

    public static void GravaCript(String Crip, String loc) { // gravacao em arquivo da Criptografado
        ...
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(loc), "UTF-8"))){
            for (int i = 0; i < Crip.length(); i++) {
                writer.write(Crip.charAt(i));
                st = "";
            }
            writer.close();
    ...
    public int A_elev_B_mod_M(int a, int b, int m) { // (A^B)mod M
        //a = 7;
        //b = 560;
        //m = 561;
        ...
        double sqR;
        double aux;
        String byts = intToBits(b); // tranforma B em binario
        int bit = 1;
        boolean entrada = false;
        for (int i = 0; i < byts.length(); i++) { // remove o zero a esquerda
            if (entrada == false && byts.charAt(i) == '1') {
                entrada = true;
            }if (entrada) {
    ...
    // Função para converter um número inteiro em uma string representando os bits
    public static String intToBits(int numero) {
        StringBuilder sb = new StringBuilder();

        for (int i = 31; i >= 0; i--) {
            // Verificando se o bit na posição i está definido
            int bit = (numero >> i) & 1;
            sb.append(bit);
        }
    ...
    public int primoEmRelacao(int numeroBase) { // pega o maior numero primo em relação a base
        int ult = 0;
        for (int i = 2; i <= numeroBase; i++) {
            if (isPrimo(i)) { // verifica de é primo
                ult = i;
            }
        }
        return ult; // retorna o maior numero numero que seja menor que o numero base
    }

    public static boolean isPrimo(int numero) { // verifica se o numero é primo
        if (numero <= 1) {
            return false;
        }
    ...
    public int functionE(int d, int z) { // Enconta o valorque satisfaça a função (e*d)%z == 1
        int e = 3;
        for (int i = 1; i < 999999999 + z; i++) {
            if ((i*d)%z == 1) {
                e = i;
                break;
            }
    ...
    public char decoNumChar(double c) {
        return (char) (c+64);
    }

    public int elev(int base, int expoente) { // eleva base com expoente
        int r = base;
        if (expoente == 0) {
            r = 1;
        }else{
